...
export async function processBuild(
  ...
  await io.rmRF("./current_build");
  await io.cp(`./marlin_${kind}`, "./current_build", {
    recursive: true,
  });
  ...
  const configuration = (
    await readFile("./current_build/Marlin/Configuration.h", "utf8")
  ).split("\n");
  updateConfiguration(configuration, build.configuration);
  await writeFile(
    "./current_build/Marlin/Configuration.h",
    configuration.join("\n"),
  );

  const configurationAdv = (
    await readFile("./current_build/Marlin/Configuration_adv.h", "utf8")
  ).split("\n");
  updateConfiguration(configurationAdv, build.configuration_adv);
  await writeFile(
    "./current_build/Marlin/Configuration_adv.h",
  ...
  if (
    (await readdir(`./current_build/.pio/build/${build.board_env}`)).find(
  ...
  await copyFile(`./current_build/.pio/build/${build.board_env}/firmware.bin`, `./assets/firmware-${buildName}.bin`);
  ...
  try {
    await copyFile(
      "./current_build/Marlin/_Bootscreen.h",
      pathJoin(configDir, "_Bootscreen.h"),
    );
    await copyFile(
      "./current_build/Marlin/_Statusscreen.h",
  ...
  return `./assets/firmware-${buildName}.bin`;
}
