...
if (devMode) {
  console.log(
    chalk.yellow(
      "Running in devMode. Repo will not be modified and releases won't be created",
    ),
  );
}
...
(async function main() {
  ...
  const latestNightly = await getLatestNightly(client);
  if (!devMode) {
  ...
  const [ignoreStable, stableBuilds] = await shouldBuild(
    latestStable,
    "stable",
    builds,
  );
  const [ignoreNightly, nightlyBuilds] = await shouldBuild(
    latestNightly,
    "nightly",
    builds,
  );
  ...
  if (!ignoreStable) {
    console.log(
      chalk.green(`downloading stable ${chalk.underline(latestStable)}`),
    );
    await $`
    wget https://api.github.com/repos/MarlinFirmware/Marlin/tarball/${latestStable}
    mkdir marlin_stable
    tar -xvf ${latestStable} -C marlin_stable --strip-components 1
    `;
  ...
  if (!ignoreNightly) {
    console.log(
      chalk.green(`downloading nightly ${chalk.underline(latestNightly)}`),
    );
    await $`
...
async function dirSetupBefore() {
  await io.cp(`${env.GITHUB_WORKSPACE!}/last_stable.json`, ".");
  await io.cp(`${env.GITHUB_WORKSPACE!}/last_nightly.json`, ".");
  await io.cp(`${env.GITHUB_WORKSPACE!}/builds`, ".", { recursive: true });
}

async function dirSetup() {
  await io.rmRF(`${env.GITHUB_WORKSPACE!}/*`);
  await io.mkdirP(`${env.GITHUB_WORKSPACE!}/assets`);
}

// copies back the updated trackers + autoGeneratedConfigs to the main repo
async function dirSetupAfter() {
  await io.cp(`./last_stable.json`, env.GITHUB_WORKSPACE!);
  await io.cp(`./last_nightly.json`, env.GITHUB_WORKSPACE!);
  await io.cp(`./autogeneratedConfigs`, env.GITHUB_WORKSPACE!, { recursive: true });
}
...
async function shouldBuild(
  ...
  try {
    console.log(
      chalk.green(`checking builds for ${chalk.underline(kind)} release`),
    );
    const trackedBuilds: {
      [key: string]: { version: string; md5: string; assetId: number };
    } = require(`${env.GITHUB_WORKSPACE!}/last_${kind}.json`);
    for (const [buildName, loadedBuild] of Object.entries(loadedBuilds)) {
      ...
      if (
        loadedBuild.active === false ||
        (loadedBuild.only && loadedBuild.only !== kind)
      ) {
      ...
      if (kind === "stable" && loadedBuild.min_version) {
        if (!semver.valid(loadedBuild.min_version)) {
          throw new Error(
            `${buildName}->min_version: ${loadedBuild.min_version} is not a valid semver string`,
          );
        }
      ...
      if (!trackedBuilds[buildName]) {
        ...
        console.log(
          chalk.cyan(`[new build added] ${chalk.underline(buildName)}`),
        );
      } else if (
        trackedBuilds[buildName] &&
        trackedBuilds[buildName].version !== latestVersion
      ) {
        ...
        trackedBuilds[buildName] &&
        md5 !== trackedBuilds[buildName].md5
      ) {
        ...
        console.log(
          chalk.cyan(`[build changed] ${chalk.underline(buildName)}`),
        );
      } else {
        //up to date
        newBuilds[buildName] = {
...
async function doBuild(
  ...
  for (const [buildName, buildDef] of Object.entries(buildDefs)) {
    ...
    const buildPath = await processBuild(
      buildName,
      buildDef.build,
      kind,
      latestVersion,
    ...
    console.log(
      `[@Debug]Add asset: buildName:${buildName}, filename:${filename}, buildPath:${buildPath}`,
    );
    assets.push({
      buildName,
      filename,
      buildPath,
      action: buildDef.action,
      assetId: buildDef.assetId,
    });
  }
  ...
  console.log(
    `[@Debug]latestVersion: ${latestVersion}, kind: ${kind}, currentDateTime: ${currentDateTime}`,
  );
  ...
  for (const asset of assets) {
    ...
    console.log(
      `[@Debug]uploading: ${asset} buildName:${asset.buildName}, filename:${asset.filename}, buildPath:${asset.buildPath}, assetId:${asset.assetId}`,
    );
    const assetId = await uploadAsset(client, uploadUrl, asset);
